...
function parsePage(pageId, page) {
    ...
    const entries = [...page.entries.values()]
          .map((entry) => parseEntry(pageId, entry, firstRequest))
          .filter((entry) => entry);
    // outcome
    return {
        page: {
...
function parseEntry(pageref, entry, firstRequest) {
    ...
    // entry started
    const wallTimeMs = (firstRequest.wallTime +
        (entry.requestParams.timestamp - firstRequest.timestamp)) * 1000;
    const startedDateTime = new Date(wallTimeMs).toISOString();
